...
srpm:
	...
	cp -r upstream-kernel/* SOURCES/
	cp $(notdir $(SPECFILE)) SOURCES/
	cp *.patch SOURCES/
	# ------ This introduces the NorNet configuration: --------
	cp config-planetlab SOURCES/
	cat SOURCES/config-planetlab >>SOURCES/kernel-local
	# ---------------------------------------------------------
	for downloaded in $(SOURCEFILES) ; do cp $$downloaded SOURCES/ ; done
	cd ..
	./rpmmacros.sh
	export HOME=$(shell pwd) ; rpmbuild $(RPMDIRDEFS) $(RPMDEFS) --nodeps -bs $(SPECFILE)


# ###### Build binary RPM files #############################################
